...
export default async function handler(req, res) {
  ...
  try {
    console.log('Veo3 image-to-video status check for requestId:', requestId);
    ...
    console.log('Veo3 image-to-video status response:', JSON.stringify(status, null, 2));
    ...
    if (status.status === 'COMPLETED' && status.response_url) {
      ...
      console.log('Veo3 image-to-video result data:', JSON.stringify(resultData, null, 2));
    ...
    console.error('Error fetching job status from Veo3 image-to-video:', error);
