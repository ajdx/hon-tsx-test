...
export default async function handler(req, res) {
  ...
  try {
    console.log('Kling 2.1 Master image-to-video status check for requestId:', requestId);
    ...
    console.log('Kling 2.1 Master image-to-video status response:', JSON.stringify(status, null, 2));
    ...
    if (status.status === 'COMPLETED' && status.response_url) {
      ...
      console.log('Kling 2.1 Master image-to-video result data:', JSON.stringify(resultData, null, 2));
    ...
    console.error('Error fetching job status from Kling 2.1 Master (image-to-video):', error);
