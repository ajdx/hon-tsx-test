...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`Processing GPT-Image-1 text-to-image: "${prompt.substring(0, 100)}..."`);
    ...
    const result = await fal.subscribe('fal-ai/gpt-image-1/text-to-image/byok', {
      ...
      onQueueUpdate: (update) => {
        if (update.status === "IN_PROGRESS") {
          console.log('GPT-Image-1 generation progress:', update.logs?.map(log => log.message));
        }
      },
    });
    ...
    console.log(`GPT-Image-1 generation successful. Result: ${generatedImageUrl}`);
    
    return res.status(200).json({ generatedImageUrl });

  } catch (error) {
    console.error('Error processing GPT-Image-1 generation:', error);
